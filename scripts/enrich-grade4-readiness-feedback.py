...
'F02': '«livres» جمع، لذلك نستخدم أداة التنكير للجمع «des»: «Ce sont des livres.»',
...
for q in all_questions:
    ...
    if qid not in E:
        ...
        q['explanation'] = E[qid]
if missing:
    raise SystemExit(f'Missing Grade 4 explanations: {missing}')

data.setdefault('quality', {})['questionSpecificExplanations'] = True

if wrapped:
    raw['content'] = json.dumps(data, ensure_ascii=False, separators=(',', ':'))
    # Preserve the source SHA; enrichment is a deployment-time transformation.
    path.write_text(json.dumps(raw, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
else:
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

print(f'Grade 4 readiness: {len(all_questions)} questions enriched with source-specific explanations')
