...
const openmoji='https://cdn.jsdelivr.net/gh/hfg-gmuend/openmoji/color/svg/';
const visuals={
...
const replacement=`function visualFor(term){const m=${mapLiteral};return m[term]||''}function wordsHtml(ws){return '<div class="filters"><button class="filter active" id="allWords">كل الكلمات</button><button class="filter" id="knownWords">أعرفها ('+state.known.length+')</button><button class="filter" id="reviewWords">أحتاج مراجعة ('+state.review.length+')</button><button class="filter" id="flashBtn">🃏 بطاقات المراجعة</button></div><div class="words" id="wordGrid">'+ws.map((x,n)=>'<div class="word has-visual"><button class="visual-button speak" data-speak="'+x[0].replaceAll('"','&quot;')+'" aria-label="استمع إلى '+x[0].replaceAll('"','&quot;')+'"><div class="vocab-visual">'+(visualFor(x[0])?'<img src="'+visualFor(x[0])+'" alt="صورة توضيحية لـ '+x[0].replaceAll('"','&quot;')+'" loading="lazy" referrerpolicy="no-referrer">':'<div class="scene">✨</div>')+'</div></button><strong>'+x[0]+'</strong><small>'+x[1]+'</small><button class="speak" data-speak="'+x[0].replaceAll('"','&quot;')+'">🔊 استمع</button><div class="actions" style="justify-content:center"><button class="filter know" data-word="'+x[0]+'">✓ أعرفها</button><button class="filter need" data-word="'+x[0]+'">↻ مراجعة</button></div></div>').join('')+'</div>'}`;
html=html.slice(0,start)+replacement+html.slice(end);
fs.writeFileSync(path,html,'utf8');
console.log('Premier Pas vocabulary cards upgraded with image visuals in '+path);
