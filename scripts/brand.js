(function(){
  ...
  function applyPrefs(){
    const p=readPrefs();document.documentElement.dataset.hmTheme=p.theme;document.documentElement.dataset.hmFont=p.fontSize;document.documentElement.dataset.hmMotion=p.reducedMotion?'reduced':'full';document.body.classList.toggle('hm-reduced-motion',!!p.reducedMotion);document.body.classList.toggle('hm-dark',p.theme==='dark');
    if(p.theme==='light') document.body.classList.remove('hm-dark');
    if(p.theme==='system'){const dark=window.matchMedia?.('(prefers-color-scheme: dark)').matches;document.body.classList.toggle('hm-dark',!!dark)}
  }
  ...
  if(/lesson\.html$/i.test(location.pathname))load('scripts/premium-lesson-enhancer.js?v=20260812-premium-interaction4');
})();
