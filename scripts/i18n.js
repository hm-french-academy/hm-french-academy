/* HM Academy — Unified multilingual runtime v4.7 — persistent language + stable selector normalization */
(function(){'use strict';
...
const labelMap={ar:['ar','العربية','Arabe','Arabic','AR'],fr:['fr','Français','French','FR','Francais'],en:['en','English','Anglais','EN']};
...
function normalizeLang(v){const s=String(v??'').replace(/\u00a0/g,' ').trim();if(!s)return null;const low=s.toLowerCase();for(const code of supported)if(labelMap[code].some(x=>String(x).toLowerCase()===low))return code;return null;}
...
function isLanguageSelect(s){if(!s||s.tagName!=='SELECT')return false;if(s.matches('[data-lang-select],[data-hms="language"],select[data-language-select]'))return true;const vals=[...s.options].map(o=>norm(o.value||o.textContent));return ['ar','fr','en'].every(c=>vals.some(v=>normalizeLang(v)===c));}
...
function bindLanguageSelectors(){document.querySelectorAll('[data-lang-select],[data-hms="language"]').forEach(s=>{if(s.dataset.hmBound)return;s.dataset.hmBound='1';s.addEventListener('change',()=>apply(s.value));});}
...
async function load(){try{const q='?v=20260821-i18n47',g=p=>fetch(p+q,{cache:'no-store'}).then(r=>r.ok?r.json():{});const [u,f,e]=await Promise.all([g('data/i18n/ui.json'),g('data/i18n/fr.json'),g('data/i18n/en.json')]);dict={ar:u.ar||{},fr:Object.assign({},u.fr||{},f||{}),en:Object.assign({},u.en||{},e||{})};loaded=true;}catch(e){}}
async function apply(t){const next=normalizeLang(t)||'ar';lang=next;try{keys.forEach(k=>localStorage.setItem(k,lang));}catch(e){}document.documentElement.lang=lang;document.documentElement.dir=lang==='ar'?'rtl':'ltr';prepareLanguageSelectors();bindLanguageSelectors();if(!loaded)await load();if(document.body)scan(document.body,lang);prepareLanguageSelectors();bindLanguageSelectors();document.querySelectorAll('[data-lang-select],[data-hms="language"]').forEach(s=>s.value=lang);window.dispatchEvent(new CustomEvent('hm:languagechange',{detail:{lang}}));window.dispatchEvent(new CustomEvent('hm:language-changed',{detail:{lang}}));}
window.HMLanguage={get:()=>lang,apply,translate:tr,dict};window.HMSetLanguage=apply;window.HMTranslate=v=>tr(v,lang);
let pending=false;const observer=new MutationObserver(ms=>{if(pending||!ms.some(m=>m.type==='childList'&&m.addedNodes.length))return;pending=true;setTimeout(()=>{pending=false;prepareLanguageSelectors();bindLanguageSelectors();const l=lang;for(const m of ms)for(const n of m.addedNodes){if(n.nodeType===3)text(n,l);else if(n.nodeType===1&&!n.closest(protectedSelector))scan(n,l);}},160);});
function mount(){prepareLanguageSelectors();bindLanguageSelectors();observer.observe(document.documentElement,{subtree:true,childList:true});apply(lang);}
