(function(){'use strict';
/* HM Academy — Unit 2 experience bridge.
 * Keeps Unit 2 lesson JSON source-aligned while exposing the same premium
 * lesson workflow used by Unit 1: interactive lesson, practice/games,
 * formal assessment and exam resources.
 */
const MAP={
...
function inject(){const cfg=MAP[id()],viewer=document.querySelector('#viewer');if(!cfg||!viewer||viewer.dataset.u2Bridge)return;viewer.dataset.u2Bridge='1';
 const existing=[...viewer.querySelectorAll('a')].map(a=>a.getAttribute('href'));
 const wrap=document.createElement('div');wrap.className='u2-premium-resources';wrap.style.cssText='padding:18px;margin:14px 0;border:1px solid #dfe6f0;border-radius:20px;background:#f8faff';
 ...
 const box=wrap.querySelector('div');[['🎮 الألعاب',cfg.games],['📝 التقييم الرسمي',cfg.assessment],['🏆 الاختبار',cfg.exam]].forEach(([label,href])=>{if(!href||existing.includes(href))return;const a=document.createElement('a');a.href=href;a.className='btn';a.textContent=label;a.style.textDecoration='none';box.appendChild(a)});
 if(cfg.interactive&&!existing.includes(cfg.interactive)){const a=document.createElement('a');a.href=cfg.interactive;a.className='btn';a.textContent='✨ الشرح التفاعلي';a.style.textDecoration='none';box.insertBefore(a,box.firstChild)}
