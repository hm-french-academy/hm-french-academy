...
for(const file of files){
  let data;
  try{data=JSON.parse(fs.readFileSync(file,'utf8'));}catch{errors.push(`${file}: invalid JSON`);continue;}
  ...
  if(data.quality?.assessmentPoolsMustBeDisjoint && data.diagnostic?.questions && data.finalAssessment?.questions){
    ...
    const ap=new Set(data.diagnostic.questions.map(q=>String(q.prompt||'').trim().toLowerCase()));
    for(const q of data.finalAssessment.questions) if(ap.has(String(q.prompt||'').trim().toLowerCase())) errors.push(`${file}: diagnostic/final duplicate prompt ${q.id}`);
  }
}
console.log(`Review QA scanned ${files.length} review-content.json file(s).`);
if(errors.length){console.error(errors.join('\n'));process.exit(1);}
console.log('Review QA passed: unique IDs, unique options, valid answers, visual-context protection, and disjoint diagnostic/final pools passed.');