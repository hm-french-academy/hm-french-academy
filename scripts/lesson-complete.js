...
(function(){
  ...
  function markCurrentLesson(){
    ...
    if(id)HMProgress.startLesson(id);
  }
  ...
  window.markLessonComplete=function(lessonId,achievementId,skill){
    ...
    var id=lessonId||currentLessonId(),before=HMProgress.get(),already=Array.isArray(before.completedLessons)&&before.completedLessons.includes(id);
    ...
    if(message)message.textContent=already?'ℹ️ هذا الدرس مكتمل بالفعل وتم الحفاظ على تقدمك.':'🎉 تم إكمال الدرس وحفظ التقدم وإضافة 50 XP.';
