...
(function(){
  ...
  function markCurrentLesson(){
    ...
    try{
      if(localStorage.getItem(legacyKey)){
        ...
        if(!done){state.completedLessons=Array.isArray(state.completedLessons)?state.completedLessons.concat([id]):[id];state.currentLessonId=id;state.currentSection='lesson';HMProgress.save(state);}
        localStorage.removeItem(legacyKey);
      }
    }catch(e){console.warn('HM Academy: legacy completion migration skipped.',e);}
    var current=HMProgress.get(),requested=new URLSearchParams(location.search).get('section');
    if(current.currentLessonId!==id)HMProgress.startLesson(id);
    if(requested)HMProgress.setSection(requested);
  }
  function bindCompletion(){
    ...
    button.onclick=null;button.__hmCompletionBound=true;
    button.addEventListener('click',function(){window.markLessonComplete(currentLessonId());});
    if(window.HMProgress){var state=HMProgress.get(),id=currentLessonId(),completed=Array.isArray(state.completedLessons)&&state.completedLessons.includes(id);if(completed){button.textContent='✅ تم إكمال الدرس';button.disabled=true;button.setAttribute('aria-pressed','true');var message=document.querySelector('#completionMessage,#completion-message,#status');if(message)message.textContent='ℹ️ هذا الدرس مكتمل بالفعل وتم الحفاظ على تقدمك.';}}
  }
