(function(){
  ...
  async function init(){
    ...
    loading=(async()=>{
      ...
      if(!window.supabase) await loadScript('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');
  ...
  async function ensureVisitorSession(){
    ...
    const r=await c.from('visitor_sessions').insert({anonymous_id:localStorage.getItem('hm_anon_id')||('a_'+crypto.randomUUID()),device_type:/Mobi|Android/i.test(nav.userAgent)?'mobile':'desktop',browser_family:nav.userAgentData?.brands?.[0]?.brand||'unknown',os_family:nav.platform||'unknown',language:navigator.language,referrer_domain:ref,landing_path:location.pathname}).select('id').single();
  ...
  async function track(event){
    ...
    try{
      const u=await user();const sid=await ensureVisitorSession();
      await c.from('analytics_events').insert({session_id:sid||null,user_id:u?.id||null,event_type:event.type,path:event.path||location.pathname,page_title:event.page||document.title,lesson_id:event.data?.lessonId||null,course_id:event.data?.courseId||null,activity_id:event.data?.activityId||null,duration_seconds:event.data?.durationSeconds||null,event_data:event.data||{}});
    }catch(e){console.debug('HM production analytics skipped',e)}
  }
