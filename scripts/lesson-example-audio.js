(function(){
  ...
  function button(host,text,label){if(host.dataset.hmAudioReady||host.querySelector('.example-speak,.hm-example-audio'))return;host.dataset.hmAudioReady='1';const b=document.createElement('button');b.type='button';b.className='hm-example-audio';b.innerHTML='🔊 <span>'+label+'</span>';b.title='استمع إلى المثال';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();speak(text,host)});host.appendChild(b)}
  function enhance(){
    document.querySelectorAll('.rule-example').forEach(el=>{const text=clean(el.querySelector('strong')?.textContent||'');if(text)button(el,text,'استمع إلى المثال')});
    document.querySelectorAll('.line').forEach(el=>{const text=clean(el.querySelector('b')?.textContent||'');if(text)button(el,text,'استمع إلى الحوار')});
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',enhance,{once:true});else enhance();new MutationObserver(enhance).observe(document.body,{childList:true,subtree:true});
})();
