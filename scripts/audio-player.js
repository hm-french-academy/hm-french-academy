...
(function(){
  ...
  function loadQualityGuard(){if(!isGrade5Lesson()||document.querySelector('script[data-hm-g5-quality]'))return;const s=document.createElement('script');s.src='scripts/grade5-quality-guard.js?build=20260819-22';s.dataset.hmG5Quality='1';document.head.appendChild(s)}
  function syncCompletion(){if(!isGrade5Lesson())return;const id=new URLSearchParams(location.search).get('id');if(!id)return;loadProgress(()=>{window.HMProgress.startLesson(id);const legacy=localStorage.getItem('hm-g5-complete-'+id)==='true';if(legacy&&!window.HMProgress.get().completedLessons.includes(id))window.HMProgress.completeLesson(id,50);const done=window.HMProgress.get().completedLessons.includes(id);const msg=document.getElementById('completionMessage'),btn=document.getElementById('completeBtn');if(done){if(msg)msg.textContent='تم إكمال الدرس وحفظ +50 XP في تقدمك العام.';if(btn){btn.textContent='✓ الدرس مكتمل';btn.disabled=true}}loadQualityGuard()})}
  function completeGrade5(e){if(!isGrade5Lesson())return;const btn=e.target.closest('#completeBtn');if(!btn)return;const id=new URLSearchParams(location.search).get('id');if(!id)return;loadProgress(()=>{window.HMProgress.completeLesson(id,50);localStorage.setItem('hm-g5-complete-'+id,'true');const msg=document.getElementById('completionMessage');if(msg)msg.textContent='تم إكمال الدرس وحفظ +50 XP في تقدمك العام.';btn.textContent='✓ تم إكمال الدرس';btn.disabled=true;loadQualityGuard()})}
