...
(() => {
  ...
  const loadPremiumCopy = () => {
    ...
    s.src = 'scripts/premium-copy-runtime.js?v=20260812-copy1';
    s.dataset.premiumCopy = '1';
  ...
  const apply = () => {
    ...
    root.querySelectorAll('.vcard .example').forEach((el,i) => {
      ...
      if (french && !el.querySelector('.example-speak')) {
        ...
        b.type = 'button'; b.className = 'example-speak'; b.textContent = '🔊 استمع إلى المثال';
    ...
    root.querySelectorAll('.resources .resource-grid article').forEach((card,i) => {
      ...
      if (!card.querySelector('.resource-kicker')) {
        const kicker = document.createElement('div'); kicker.className = 'resource-kicker';
    ...
    root.querySelectorAll('.card').forEach(card => {
      ...
      const visual = document.createElement('div'); visual.className = 'video-identity';
      visual.innerHTML = '<div class="video-identity-art">🎬</div><div><strong>مشهد الفصل · À l’école</strong><span>شاهد الموقف، استمع إلى الحوار، ثم طبّق ما تعلمته في المفردات والقاعدة.</span></div>';
