...
(function(){
  ...
  function speak(text,options={}){
    ...
    const synth=window.speechSynthesis,button=options.button||null,lang=options.lang||DEFAULT_LANG,rate=Math.max(.55,Math.min(1.15,Number(options.rate)||DEFAULT_RATE));
    try{
      if(current){try{current.cancel();}catch(_){}current=null;} synth.cancel();
      const u=new SpeechSynthesisUtterance(value);u.lang=lang;u.rate=rate;u.pitch=Number(options.pitch)||1;const voice=pickVoice(lang);if(voice)u.voice=voice;state(button,true);
      return new Promise(resolve=>{let settled=false;const finish=ok=>{if(settled)return;settled=true;state(button,false);if(current===u)current=null;resolve(ok);};u.onend=()=>finish(true);u.onerror=()=>finish(false);current=u;try{synth.resume();}catch(_){}synth.speak(u);window.setTimeout(()=>{try{if(synth.paused)synth.resume();}catch(_){}},120);window.setTimeout(()=>finish(true),Math.max(3000,Math.min(15000,value.length*140+1800)));});
  ...
  function applyLessonVisualTheme(){
    ...
    const p=new URLSearchParams(location.search),id=p.get('id')||'grade8-u1-l1';
    const themes={
      'grade8-u1-l1':{icon:'🏫',label:'في المدرسة',a:'#173a82',b:'#2563eb',c:'#16a394'},
      'grade8-u1-l2':{icon:'✏️',label:'الأدوات المدرسية والمقلمة والشنطة',a:'#243b8f',b:'#4169e1',c:'#2aa6a1'},
      'grade8-u1-l3':{icon:'📚',label:'المواد الدراسية وتصريف الأفعال',a:'#312e81',b:'#7c3aed',c:'#2563eb'},
      'grade8-u1-l4':{icon:'👩‍⚕️',label:'المهن وقاعدة Il y a',a:'#134e4a',b:'#0f766e',c:'#14b8a6'},
      'grade8-u2-l1':{icon:'🛏️',label:'غرف المنزل وغرفة النوم',a:'#075e54',b:'#129b8b',c:'#4caf8f'},
      'grade8-u2-l2':{icon:'🛋️',label:'غرفة المعيشة والمطبخ والحمام',a:'#17633b',b:'#2e9b62',c:'#43b883'},
      'grade8-u2-l3':{icon:'🔤',label:'المفردات وتصريف Avoir وÊtre',a:'#7a4b13',b:'#b87920',c:'#d7a64a'},
      ...
      'grade8-u3-l2':{icon:'🧳',label:'أدوات السفر والأشخاص والتعبيرات',a:'#51266f',b:'#8847b6',c:'#d05aa0'},
    ...
    const t=themes[id]||themes['grade8-u1-l1'];hero.dataset.lessonTheme=id;hero.style.background=`linear-gradient(135deg,${t.a},${t.b},${t.c})`;hero.style.boxShadow=`0 20px 50px ${t.b}33`;art.textContent=t.icon;art.setAttribute('aria-label',t.label);art.title=t.label;
  }
  ...
  const observer=new MutationObserver(()=>applyLessonVisualTheme());
