(function(){
  ...
  function loadEnhancements(){
    ...
    s.src='scripts/grade9-u1-l1-enhancements.js?v=20260911-3';
    s.defer=false;
  ...
  function boot(){
    ...
    // The current standard lesson already contains its complete, working renderer.
    // Never replace that renderer with the emergency fallback when it is present.
    if(typeof window.go==='function' && typeof window.speak==='function'){
      loadEnhancements();
      return;
    }

    // Emergency fallback is retained only for a genuinely missing lesson runtime.
    ...
    function speak(t){try{if(!window.speechSynthesis)return;window.speechSynthesis.cancel();var u=new SpeechSynthesisUtterance(t);u.lang='fr-FR';u.rate=.78;window.speechSynthesis.speak(u)}catch(e){}}
    function render(){
      ...
      if(idx===1)h='<h2>🖼️ المفردات</h2><p>استمع إلى الكلمة أو المثال من نفس البطاقة.</p><div class="cards">'+vocab.map(function(x){return '<article class="card"><div class="sport-art">'+x[2]+'</div><div class="fr">'+esc(x[0])+'</div><div class="ar">'+esc(x[1])+'</div><div class="example">'+esc(x[3])+'</div><button class="btn primary" data-s="'+esc(x[0])+'">🔊 نطق الكلمة</button><button class="btn secondary" data-s="'+esc(x[3])+'">▶ نطق المثال</button></article>'}).join('')+'</div>';
      if(idx===2)h='<h2>🎧 النطق</h2><p>استمع إلى الكلمات وراجع أصواتها الفرنسية.</p><div class="pron-grid">'+vocab.map(function(x){return '<article class="pron-card"><div class="fr">'+esc(x[0])+'</div><div class="ar">'+esc(x[1])+'</div><div class="note"><b>💡 ملاحظة:</b><br>انتبه إلى نطق الحروف الفرنسية المتحركة والأصوات الأنفية.</div><button class="btn primary" data-s="'+esc(x[0])+'">🔊 استمع للكلمة</button></article>'}).join('')+'</div>';
      ...
      if(idx===4)h='<h2>💬 المحادثة</h2><p>اقرأ الحوار ثم استمع إلى السطر الذي تريد سماعه.</p><div class="dialogue-wrap"><article class="dialogue-card sara"><div class="speaker">Sara</div><div class="line-fr">Quel sport pratiques-tu ?</div><div class="line-ar">ما الرياضة التي تمارسها؟</div><button class="btn primary" data-s="Quel sport pratiques-tu ?">🔊 استمع</button></article><article class="dialogue-card adam"><div class="speaker">Adam</div><div class="line-fr">Je pratique le football.</div><div class="line-ar">أنا أمارس كرة القدم.</div><button class="btn primary" data-s="Je pratique le football.">🔊 استمع</button></article></div>';
      ...
      panel.innerHTML=h;
      panel.querySelectorAll('[data-s]').forEach(function(b){b.onclick=function(){speak(b.getAttribute('data-s'))}});
