...
async function loadBank(path){for(const u of ['./'+path,'https://raw.githubusercontent.com/hm-french-academy/hm-french-academy/main/'+path]){try{const r=await fetch(u+'?v=20260903-auth-diagnostic',{cache:'no-store'});if(!r.ok)continue;let x=await r.text();try{x=JSON.parse(x)}catch{}if(x?.encoding==='base64')x=decode(x.content);else if(x?.content)x=x.content;x=deep(x);if(x?.diagnostic?.questions)return x}catch{}}throw new Error('diagnostic bank unavailable')}
(async()=>{
  ...
  btn.onclick=async()=>{
    ...
    try{
      ...
      const record={version:2,target,score,total:diag.length,completedAt:new Date().toISOString()};
